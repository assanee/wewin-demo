...
const TH: Readonly<Record<string, Renderer>> = {
  'order.submitted_for_payment.customer': customer(
    ...
    (c) =>
      `เราได้รับคำขอใบเสนอราคาของท่านเรียบร้อยแล้ว (${orderLabel(c)}) และได้บันทึกรายละเอียดสินค้า ราคา และภาษีมูลค่าเพิ่มไว้ตามที่ท่านเห็น\n` +
      'ยอดที่แสดงเป็น **ราคาประมาณ** — ทีมงานจะตรวจสอบและยืนยันอีกครั้ง แล้วแจ้งท่านทราบ จึงจะชำระเงินได้ ' +
      'ยังไม่ต้องโอนเงินในขั้นตอนนี้' +
      documentLink(c, 'ดูใบเสนอราคาฉบับเต็มได้ที่ลิงก์นี้ (บันทึกหรือสั่งพิมพ์ได้)'),
  ),
  'order.submitted_for_payment.sales': staff(
    (c) => `[ใหม่] รอยืนยันใบเสนอราคา — ${orderLabel(c)}`,
    (c) =>
      `มีคำขอใบเสนอราคาใหม่รอการยืนยัน: ${orderLabel(c)}\n` +
      'โปรดตรวจรายการและราคา ปรับยอดมัดจำหรือส่วนลดถ้าจำเป็น แล้วกดยืนยัน — ลูกค้าจะชำระเงินได้หลังจากนั้น',
  ),
