...
> เฟส 1 ทำไว้สี่ที่แล้ว: `orders` (ทั้งรายการและรายละเอียด) · `quotes` · หน้าภาพรวม · `account`
> อีกสิบเอ็ดหน้าจอที่เหลือคือเฟส 2 — **งานที่เหลือคือเอากฎข้างล่างนี้ไปใช้ ไม่ใช่คิดใหม่**

เจ้าของบอกว่าทุกหน้า **ลายตา** และ **ไม่มีจุดเด่น** ซึ่งวัดได้ ไม่ใช่เรื่องรสนิยม:

- `CardTitle` ใน `ui/card.tsx` ตั้งค่าเริ่มต้นเป็น `text-base` (16px) และ `Card` ตั้ง `text-sm`
  (14px) ให้ทุกอย่างข้างใน → **หัวข้อใหญ่กว่าเนื้อหาของตัวเองแค่ 2px ใน 39 จาก 41 จุด**
  ที่มี `<CardTitle>` ในแอปนี้ นั่นไม่ใช่ลำดับชั้นที่อ่อน แต่คือไม่มีลำดับชั้น
...
| หน้ารายการ | **ตัวรายการเอง** — เอากรอบออก บีบแถวให้แน่นขึ้น |

⚠️ **`CardTitle` ยังตั้งค่าเริ่มต้นเป็น `text-base`** เปลี่ยนคำเดียวใน `ui/card.tsx` เป็น
`type-section` จะแก้อีกสิบเอ็ดหน้าจอในบรรทัดเดียว — และจะ *เปลี่ยนหน้าตา* ทั้งสิบเอ็ดหน้าโดยที่
ยังไม่มีใครดูมันเลย ซึ่งคือเฟส 2 ที่มาถึงโดยผลข้างเคียง **เป็นการตัดสินใจของเจ้าของ**
