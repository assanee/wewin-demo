...
## ⭐ ลำดับความสำคัญของหน้าจอ — สเกลตัวอักษร และกฎว่าอะไรได้ `Card`

> **ใช้ครบทั้ง 15 พื้นที่แล้ว** (เฟส 1 สี่ที่ · เฟส 2 อีกสิบเอ็ด) หน้าจอใหม่ต้องทำตามกฎนี้
> ไม่ใช่คิดใหม่ ถ้าจะออกนอกกฎ ให้แก้กฎที่นี่ก่อน แล้วค่อยแก้โค้ด

เจ้าของบอกว่าทุกหน้า **ลายตา** และ **ไม่มีจุดเด่น** ซึ่งวัดได้ ไม่ใช่เรื่องรสนิยม:

- `CardTitle` ใน `ui/card.tsx` **เคย** ตั้งค่าเริ่มต้นเป็น `text-base` (16px) และ `Card` ตั้ง
  `text-sm` (14px) ให้ทุกอย่างข้างใน → **หัวข้อใหญ่กว่าเนื้อหาของตัวเองแค่ 2px ใน 39 จาก 41 จุด**
  ที่มี `<CardTitle>` ในแอปนี้ นั่นไม่ใช่ลำดับชั้นที่อ่อน แต่คือไม่มีลำดับชั้น (แก้แล้วในเฟส 2 —
  ดูหัวข้อ `CardTitle` ข้างล่าง)
- หน้าภาพรวมเรนเดอร์ `<Card>` **สิบใบ** บนบัญชีที่มีสิทธิ์ครบ (ในไฟล์เขียนไว้ห้าจุด แต่
  `QueueTile` หนึ่งจุดเรนเดอร์ซ้ำตามจำนวนคิว) ความรบกวนมาจาก **กรอบ ไม่ใช่เนื้อหา**

### สเกล — ใช้ห้าคลาสนี้ ห้ามเขียน `text-*` ขนาดตรงๆ ในหน้าจอใหม่

| คลาส | ขนาด | ใช้กับ |
|---|---|---|
| `type-page` | 30px / 600 | ชื่อหน้า — **`PageHeader` เท่านั้น หน้าละหนึ่งครั้ง** |
| `type-focal` | 24px / 600 | ประโยคหลักของหน้า — **หน้าละไม่เกินหนึ่งครั้ง** |
| `type-section` | 18px / 600 | หัวข้อ section และเป็นค่าที่ `CardTitle` ควรเป็น |
| `type-body` | 14px / 400 | เนื้อหา |
| `type-caption` | 12px / 400 | ข้อมูลกำกับ คำอธิบายใต้ตัวเลข หัวคอลัมน์ตาราง |

นิยามอยู่ใน `src/app/globals.css` พร้อมเหตุผล — อ่านคอมเมนต์ก่อนแก้ค่า สองข้อที่สำคัญ:
**ไม่มีอะไรอยู่ที่ 16px อีกแล้ว** (16 คือตัวที่ไปชนกับ 14) และ **ห้ามใส่ `letter-spacing` ติดลบ
ทุกขนาด** เพราะไทยซ้อนสระบนบรรทัดและวรรณยุกต์บนสระอีกชั้น

### กฎว่าอะไร "ได้" `Card`

`Card` **ไม่ใช่คอนเทนเนอร์เริ่มต้นอีกแล้ว** ถามสามข้อนี้ก่อนใส่:

1. **มันแยกออกไปอ่านเองได้ไหม** — รายละเอียดผู้ติดต่อ ตารางแยกยอดเงิน ชุดตัวเลือกหนึ่งชุด
   ถ้าคัดลอกไปวางที่อื่นแล้วยังอ่านรู้เรื่อง → ได้ `Card`
2. **มันจะสับสนกับของข้างๆ ไหมถ้าไม่มีกรอบ** — ถ้าไม่ กรอบนั้นคือการยืนยันสิ่งที่ไม่มีใครสงสัย
   ระยะห่าง (`gap-8` / `gap-10`) กับ `type-section` แยกได้อยู่แล้ว
3. **มันวาดขอบของตัวเองอยู่แล้วหรือเปล่า** — `<Table>` มีเส้นคั่นครบ; ครอบด้วย `Card`
   คือวาดขอบรอบขอบ **ห้ามใส่ `Card` รอบตาราง**

ตัวอย่างที่ตัดสินแล้ว:

| ได้ `Card` | ไม่ได้ — ใช้ `<section>` + `type-section` |
|---|---|
| ผู้ติดต่อ / ยอดเงิน (order) | ตารางออเดอร์ · ตารางใบเสนอราคา · ตารางรายการ |
| ยอดรวม / อำนาจอนุมัติ (quote) | สันเหตุการณ์ของออเดอร์ · ตรวจก่อนส่ง |
| เงิน (ภาพรวม) | คิวงาน · ออเดอร์ · แคตตาล็อกและระบบ (ภาพรวม) |
| `PrincipalCard` (diagnostic ที่พับไว้) | ทุก section ของ `/account` — **ไม่มี `Card` เหลือเลย** |

⚠️ **นับการ์ดที่ *เรนเดอร์* ไม่ใช่ที่เขียนไว้ในไฟล์** `<Card>` ที่อยู่ใน `.map()` นับหนึ่งใบ
ต่อหนึ่งรายการ ภาพรวมเขียนไว้ห้าจุดแต่เรนเดอร์สิบใบ · `/option-groups` เขียนจุดเดียวแต่เรนเดอร์
ประมาณสิบใบ · แท็บกลุ่มใน `/users` เรนเดอร์หนึ่งใบต่อหนึ่งกลุ่ม **การ์ดเหมือนกัน N ใบเรียงกันคือ
อาการเดียวกับที่เจ้าของบ่น** พอทุกอย่างเท่ากันหมด ก็ไม่มีอะไรเด่น — ใช้ `divide-y divide-border/60`
แทน แล้วให้หัวข้อแต่ละรายการเป็น `type-section`

### "หนึ่งหน้า หนึ่งจุดเด่น" และจุดเด่นนั้นไม่ใช่ `Card`

สิ่งที่คุณเปิดหน้านั้นมาดูต้องอยู่บนพื้นหน้า ไม่มีกรอบ ที่ `type-focal` และตรรกะที่หามันควรอยู่ใน
`.ts` ที่เทสต์ได้ (`overview-focus.ts`, `order-focus.ts`) เพราะ vitest ที่นี่เป็น
`environment: 'node'` — **`.test.tsx` จะไม่ถูกเก็บไปรัน** เงียบๆ

#### ⭐ กฎที่เฟส 2 สอน: จุดเด่นต้องพูดสิ่งที่อ่านจากที่อื่นบนหน้าเดียวกันไม่ได้

นี่คือบททดสอบข้อเดียวที่ใช้ได้จริง **ถ้าตัวเลขหรือประโยคนั้นมีอยู่แล้วที่อื่นบนหน้าจอเดียวกัน
มันไม่ใช่จุดเด่น มันคือของซ้ำที่ใส่เสื้อจุดเด่น** จุดเด่นที่ได้ผลทุกอันในแอปนี้เป็น *ข้อสรุป*
ที่คำนวณมา ไม่ใช่ตัวเลขดิบที่ยกมาวางซ้ำ

เฟส 1 เคยเปิดหน้า quote ด้วยยอดรวมสุทธิที่ `type-focal` แล้ว `TotalsCard` ก็แสดงยอดเดียวกัน
ใต้ป้ายเดียวกันอีกครั้งห่างลงไป 400px — **ตอนนี้เอาออกแล้ว** และหน้านั้นไม่มี `type-focal` เลย
ซึ่งถูกต้อง เพราะสิ่งที่คุณเปิดหน้านั้นมาทำคือ *แก้ตารางรายการ* ตารางจึงเป็นจุดเด่นอยู่แล้ว

| หน้า | จุดเด่น |
|---|---|
| ภาพรวม | มีงานค้างอยู่เท่าไร และอยู่คิวไหน (`overviewFocus`) |
| order | สถานะปัจจุบัน + เดินต่อได้กี่ทาง (`orderFocus`) |
| account | บัญชีนี้คือใคร (จำนวนทางเข้าอยู่ใน `Alert` เมื่อเหลือทางเดียว) |
| **หน้ารายการ / หน้าแก้ไข** | **ตัวรายการเอง — ไม่ต้องมี `type-focal`** เอากรอบออก บีบแถวให้แน่น แล้วปล่อยให้ตารางเป็นของชิ้นแรกที่ไม่มีกรอบใต้หัวเรื่อง |

⚠️ ไม่มี `type-focal` **ดีกว่า** `type-focal` ที่พูดซ้ำของที่อยู่ข้างล่าง

### `CardTitle` เป็น `type-section` แล้ว — ห้ามใส่คลาสขนาดทับ

เฟส 2 เปลี่ยนค่าเริ่มต้นใน `ui/card.tsx` จาก `text-base` เป็น `type-section` **`<CardTitle>`
ที่ไม่ใส่ className จึงถูกต้องอยู่แล้ว** ใส่ได้เฉพาะคลาส layout (`flex`, `gap-2`) เท่านั้น
ตอนเปลี่ยนได้ลบคลาสที่พูดซ้ำค่าเริ่มต้นออกไป 12 จุด

⚠️ `group-data-[size=sm]/card:text-sm` ถูกลบไปพร้อมกัน — การ์ดแบบ `size="sm"` เคยลดหัวข้อลงเหลือ
14px ซึ่งเท่ากับเนื้อหาข้างใต้พอดี **การ์ดที่แน่นขึ้นคือเหตุผลให้ลด padding ไม่ใช่ให้ลดหัวข้อ
จนไม่เป็นหัวข้อ** ความแน่นอยู่ที่ `--card-spacing` อยู่แล้ว

⚠️ **`border-*` เพียวๆ บน `Card` ไม่วาดอะไรเลย** `Card` ใช้ `ring-1 ring-foreground/10` และไม่มี
border-width; preflight ของ Tailwind ตั้ง `border-width: 0` ทุก element ดังนั้น
`border-foreground/20` คือ CSS ที่ตายแล้ว — เจอสองจุดที่เขียนไว้แบบนั้นและไม่เคยแสดงผล ใช้
`ring-*` แทน
