...
export const en: PartialUiCatalogue = {
  ...
  'payment.form.imageChoose': 'Choose a photo',
  'payment.form.imageChange': 'Change photo',
  ...
  'payment.history.submitted': (p, f) => `${f.bahtExact(p.slipMinor)} · sent ${f.date(p.sentAt)} · being checked`,
  'payment.history.accepted': (p, f) => `${f.bahtExact(p.slipMinor)} · sent ${f.date(p.sentAt)} · accepted`,
  'payment.history.rejected': (p, f) => `${f.bahtExact(p.slipMinor)} · not accepted — ${p.reason}`,
