-- ─────────────────────────────────────────────────────────────────────────────
-- ⭐ ERASURE REACHES THE TAX AND PROFILE-HISTORY ACTORS
-- ─────────────────────────────────────────────────────────────────────────────
--
-- `tax_countries.updated_by_user_id`, `tax_country_changes.changed_by_user_id` and
-- `organisation_profile_changes.changed_by_user_id` each name the member of *staff* who
-- last touched the company's tax policy or its own change history — never the customer
-- being erased. The FK is `ON DELETE SET NULL`, which fires only on a literal
-- `DELETE FROM users`, and erasure does not delete the user row; it scrubs and closes.
-- So `erase_user()` has to null these three explicitly, the same way 0028 already nulls
-- the three organisation actors. See `ERASURE_TREATMENTS` in src/schema/auth.ts and
-- packages/db/tests/erasure.test.ts.
--
-- `erase_user()`'s body below is carried **unchanged** from `pg_proc.prosrc` as dumped
-- after 0028, plus exactly the three `UPDATE` statements in the new "tax and profile
-- actors" block near the end. Nothing else in this function was rewritten.
